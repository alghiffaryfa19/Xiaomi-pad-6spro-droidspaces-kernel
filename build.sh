...
readonly -a ENABLED_CONFIGS=(
  ...
  # Droidspaces configs
  SYSCTL NAMESPACES SECCOMP SECCOMP_FILTER
  CGROUPS CGROUP_DEVICE CGROUP_PIDS MEMCG CGROUP_SCHED FAIR_GROUP_SCHED CGROUP_FREEZER CGROUP_NET_PRIO
  OVERLAY_FS
  FW_LOADER FW_LOADER_USER_HELPER FW_LOADER_COMPRESS
  VETH BRIDGE NETFILTER BRIDGE_NETFILTER NETFILTER_ADVANCED NF_CONNTRACK IP_NF_IPTABLES IP_NF_FILTER NF_NAT NF_TABLES IP_NF_TARGET_MASQUERADE NETFILTER_XT_TARGET_MASQUERADE NETFILTER_XT_TARGET_TCPMSS NETFILTER_NETLINK NF_NAT_REDIRECT IP_ADVANCED_ROUTER IP_MULTIPLE_TABLES NF_NAT_IPV4 IP_NF_NAT
)
